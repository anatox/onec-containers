#!/usr/bin/env bash
# https://github.com/reviewdog/action-shellcheck/blob/master/script.sh
# Внесены исправления для корректной работы fail-level, не ломающей работу suggestion
...
if [ "${INPUT_CHECK_ALL_FILES_WITH_SHEBANGS}" = "true" ] && (( ${#names[@]} )); then
...
if [ "${INPUT_REPORTER}" = 'github-pr-review' ]; then
  ...
  shellcheck -f json  ${INPUT_SHELLCHECK_FLAGS:-'--external-sources'} ${FILES} \
    | jq -r '.[] | "\(.file):\(.line):\(.column):\(if .level == "style" then "info" else .level end):\(.message) [SC\(.code)](https://github.com/koalaman/shellcheck/wiki/SC\(.code))"' \
    | reviewdog \
        -efm="%f:%l:%c:%t%*[^:]:%m" \
        -name="shellcheck" \
...
# sk: suggestion pass should never fail — diffs have no severity
# sk: without fail-level=none, would always be "error"; exit code is ignored
# shellcheck disable=SC2086
