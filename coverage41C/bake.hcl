target "coverage-agent-k8s" {
  ...
  labels = labels("base-jenkins-coverage-agent", COVERAGE41C_VERSION, "Coverage41C Jenkins agent")
...
target "coverage-agent-swarm" {
  ...
  labels = labels("base-jenkins-coverage-agent", COVERAGE41C_VERSION, "1C:Enterprise code coverage agent")
  cache_from = cache_from("base-jenkins-coverage-agent")
  cache_to = cache_to("base-jenkins-coverage-agent")
}
