...
variable "NLS_ENABLED" { default = "1" }
...
function "cache_from" {
  params = [name]
  result = REGISTRY_PREFIX != "" && CACHE_TO != "" ? [
    "type=registry,ref=${REGISTRY_PREFIX}/cache/${name}:${CACHE_TO}",
    "type=registry,ref=${REGISTRY_PREFIX}/cache/${name}:main",
  ] : []
}
...
function "cache_to" {
  params = [name]
  result = REGISTRY_PREFIX != "" && CACHE_TO != "" ? [
    "type=registry,ref=${REGISTRY_PREFIX}/cache/${name}:${CACHE_TO},mode=max"
  ] : []
}
