...
@task(
    help={
        "target": "Целевые образы или группы для сборки (через запятую)",
    },
)
...
def plan(c, target: str | None = None) -> None:  # type: ignore[no-untyped-def]
    """Вывод JSON-плана bake в stdout."""
    targets = [t.strip() for t in target.split(",") if t.strip()] if target else None
...
def select(  # type: ignore[no-untyped-def]
    ...
    if github_output:
        ...
        gh_output("has-publish", result.has_publish)
        ...
        print(f"has_publish={result.has_publish}")
...
class BakeProgram(Program):
    """Программа invoke для утилиты bake."""

    def parse_core_args(self) -> None:  # noqa: D102
        super().parse_core_args()
        # Перенаправляем --help в справку ядра.
        unparsed = self.core.unparsed
        if any(t in ("--help", "-h") for t in unparsed):
            task_name = next(
                (t for t in unparsed if t not in ("--help", "-h") and not t.startswith("-")),
                None,
            )
            if task_name and self.namespace and task_name in self.namespace.tasks:
                self.core.unparsed = []
                self.args["help"].value = task_name

    def execute(self) -> None:  # noqa: D102
        global _extra
        _extra = [a for a in shlex.split(self.core.remainder) if a not in ("--help", "-h")]
        super().execute()
