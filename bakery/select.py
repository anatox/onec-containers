...
@dataclass
class SelectResult:
    """Результат выбора целей: build-set, publish-матрица и их индикаторы."""

    all: bool
    """Признак полной пересборки (build-all)."""
    build: list[str]
    """Имена целей, требующих пересборки."""
    publish: list[dict[str, str]]
    """Матрица публикации (build ∩ группа publish)."""
    has_targets: str = "false"
    """«true», если build непустой."""
    has_publish: str = "false"
    """«true», если publish непустой."""
...
def _filter_by_pattern(targets: list[str], pattern: str) -> list[str]:
    ...
    return fnmatch.filter(targets, pattern)
...
def select(
    ...
    *,
    ...
    if not targets:
        return SelectResult(all=True, build=[], publish=[])
    ...
    if build_all:
        ...
        return SelectResult(
            ...
            build=all_targets,
            publish=publish,
            has_targets="true" if all_targets else "false",
            has_publish="true" if publish else "false",
        )

    if not changed:
        return SelectResult(all=False, build=[], publish=[])
    ...
    return SelectResult(
        ...
        build=build_set,
        publish=publish,
        has_targets="true" if build_set else "false",
        has_publish="true" if publish else "false",
    )
