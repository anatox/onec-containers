...
target "edt-toolbox" {
  ...
  labels = labels("edt-toolbox", EDT_VERSION, "1C:Enterprise Development Tools toolbox")
  description = jsonencode({"extra-srcs" = ["scripts/install-edt-jdk.sh", "scripts/install-edt-nginx.sh", "scripts/distrobox-shims.sh"]})
...
target "edt-toolbox-client" {
  ...
  labels = labels("edt-toolbox", "${EDT_VERSION}-client${ONEC_VERSION}", "1C:Enterprise Development Tools + client")
  cache_from = cache_from("edt-toolbox")
  cache_to = cache_to("edt-toolbox")
}
