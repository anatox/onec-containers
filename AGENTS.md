...
Build system: `docker buildx bake` with HCL configuration. Targets in `*/bake.hcl` alongside Dockerfiles, shared variables/functions/groups in `bakery/`. Inter-image dependencies via named contexts. Selector (`bakery/select.py`) computes affected-closure from git diff and context graph. CLI subcommands via invoke: `./bake build`, `./bake plan`, `./bake select`, `./bake lint`.

## Bake

Full reference: `bakery/AGENTS.md` — selector implementation, CI internals, invoke tasks, mypy.
...
- COPY parsing is not done automatically: `onec.extra-srcs` is a manual escape hatch for cross-dir inputs dependency referencing.

### reviewdog lint workflow

- **Shellcheck suggestion-pass bug**: `reviewdog/action-shellcheck@v1` runs two internal passes: main `-f checkstyle` + suggestion `-f diff`. The `-f=diff` format has no per-finding severity, so reviewdog defaults all findings to `error` — causing `fail_level: error` to fail on warnings/info from the suggestion pass. The action script runs both passes unconditionally with the same `INPUT_FAIL_LEVEL`, offering no way to disable the suggestion pass or set a different fail level for it.
- **Resolution**: Replaced with `.github/actions/shellcheck` (composite, same parameter interface). The main checkstyle pass still respects `fail_level`. The suggestion pass hardcodes `-fail-level=none` — it posts PR review suggestions but never fails the step.
- `-f checkstyle` XML output also has no severity per-finding; reviewdog falls back to `-level` flag for all findings. With `level: error`, shellcheck `SC2034`/`SC2155` (native warnings) are reported as error annotations.
...
- **Python**: `bakery/buildx.py`, `bakery/plan.py`, `bakery/select.py`, `bakery/main.py` (~500 lines)
