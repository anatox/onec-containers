...
```bash
./bake.py server          # один образ
./bake.py default         # все образы
REGISTRY_PREFIX=ghcr.io/user ./bake.py --push server  # публикация
```
...
- Python 3 для `./bake.py` и `bake/select.py`
...
```bash
./bake.py client-toolbox      # 1С клиент + toolbox
./bake.py edt-toolbox         # EDT + toolbox
./bake.py edt-toolbox-client  # EDT + 1С клиент + toolbox
```
