target "base-jenkins-agent-k8s" {
  ...
  tags = agent_tags("base-jenkins-agent", ONEC_VERSION, "k8s")
  labels = labels("base-jenkins-agent", "1C base Jenkins agent ${ONEC_VERSION} (k8s)")
...
target "edt-agent-k8s" {
  ...
  tags = agent_tags("edt-agent", EDT_VERSION, "k8s")
  labels = labels("edt-agent", "EDT Jenkins agent ${EDT_VERSION} (k8s)")
  cache_from = cache_from("edt-agent")
  cache_to = cache_to("edt-agent")
}

target "oscript-agent-k8s" {
  dockerfile = "k8s-jenkins-agent/Dockerfile"
  context = "."
  contexts = {
  ...
  tags = agent_tags("oscript-agent", ONESCRIPT_VERSION, "k8s")
  labels = labels("oscript-agent", "OneScript Jenkins agent ${ONESCRIPT_VERSION} (k8s)")
